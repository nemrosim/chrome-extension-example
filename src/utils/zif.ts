...
class ZoomLevel {
    private file: LocalFile;
    private meta: Map<any, any>
    private tilesInfos: Promise<any>;
...
export class LocalFile {
    ...
    getBytes = (begin, end): Promise<CustomBytes> => {
        return new Promise<CustomBytes>((resolve, reject) => {
            /**
            ...
            fileReader.onload = (evt) => {
                resolve(new CustomBytes(fileReader.result));
            }
            fileReader.onerror = (evt) => {
                reject(fileReader.error);
            };
            fileReader.readAsArrayBuffer(this.file.slice(begin, end));
        });
    }
}
